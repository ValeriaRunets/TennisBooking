...
async def test_alert_sent_for_new_slot(tmp_path):
    ...
    slots = {URL: [_slot(18)]}
    checker, _, bot = _make_checker(tmp_path, [link], slots)

    await checker.check_all_links(bot)

    bot.send_message.assert_awaited_once()
    assert "18:00" in bot.send_message.call_args.kwargs["text"]
...
async def test_alert_again_after_slot_disappears_and_returns(tmp_path):
    ...
    await checker.check_all_links(bot)  # alert
    slots[URL] = [_slot(18, available=False)]  # someone booked it
    await checker.check_all_links(bot)  # no alert
    slots[URL] = [_slot(18)]  # it freed up again
    await checker.check_all_links(bot)  # alert again
...
async def test_only_new_slots_in_second_alert(tmp_path):
    ...
    assert bot.send_message.await_count == 2
    second_msg = bot.send_message.call_args.kwargs["text"]
...
async def test_notified_keys_persisted(tmp_path):
    link = MonitoredLink(url=URL)
    slots = {URL: [_slot(18)]}
    checker, mgr, bot = _make_checker(tmp_path, [link], slots)
...
async def test_failure_warning_after_threshold_sent_once(tmp_path):
    ...
    assert bot.send_message.await_count == 1
    assert "Trouble" in bot.send_message.call_args.kwargs["text"]
    saved = mgr.load().links[0]
